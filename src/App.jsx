...
export function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="side-bottom"><a>设置</a><a>集成</a><a>文档</a></div>
    </aside>
    <section className="workspace">
      <header className="topbar"><div className="project-wrap"><button className={`project ${projectOpen ? "open" : ""}`} onClick={() => setProjectOpen((open) => !open)} aria-haspopup="menu" aria-expanded={projectOpen}><span className="project-mark"></span>{project}<span className="chevron">⌄</span></button>{projectOpen && <div className="project-menu" role="menu"><p>项目</p>{["Payments", "Storefront", "Mobile app"].map((name) => <button role="menuitem" className={project === name ? "chosen" : ""} key={name} onClick={() => { setProject(name); setProjectOpen(false); setToast(`已切换到 ${name} 项目。`); }}><span className="project-mark"></span>{name}{project === name && <b>✓</b>}</button>)}<div className="project-divider"></div><button className="new-project" onClick={() => { setProjectOpen(false); setToast("新项目创建器将在下一版加入。"); }}>+ 添加项目</button></div>}</div><label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索任务…" /></label><div className="user">JD⌄</div></header>
