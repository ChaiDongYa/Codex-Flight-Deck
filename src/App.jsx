...
export function App() {
  ...
  return <div className="app-shell">
    ...
    {projectModalOpen && <div className="modal-backdrop" role="presentation"><section className="modal project-modal" role="dialog" aria-modal="true" aria-labelledby="project-title"><button className="modal-close" onClick={() => setProjectModalOpen(false)}>×</button><div className="modal-kicker">接入本机仓库</div><h2 id="project-title">添加真实 Git 项目</h2><p className="description">选择已有 Git 仓库。Flight Deck 会校验路径；后续任务会在该仓库创建独立 worktree。</p><div className="form-stack"><label>项目文件夹绝对路径<input autoFocus value={projectPath} onChange={(event) => setProjectPath(event.target.value)} placeholder="/Users/你的用户名/code/项目名" /></label></div><footer className="modal-actions"><button className="outline" onClick={() => setProjectModalOpen(false)}>取消</button><span></span><button className="primary" onClick={addProject}>验证并添加</button></footer></section></div>}
