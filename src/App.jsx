...
export function App() {
  ...
  const visibleTasks = useMemo(() => tasks.filter((task) => (allProjects || task.projectPath === project?.path) && (filter === "全部" || task.status === filter) && `${task.title} ${task.id}`.toLowerCase().includes(query.toLowerCase())), [tasks, filter, query, allProjects, project]);
  ...
  const deleteSelectedTask = async () => { if (!window.confirm(`删除任务“${selected.title}”？\n\n只会删除 Flight Deck 本地任务记录和依赖关系，不会删除项目文件、Git 分支或 worktree。`)) return; const response = await fetch(`/api/tasks/${selected.id}`, { method: "DELETE" }); const data = await response.json(); if (!response.ok) return setToast(data.error); setTasks(data.tasks); setSelectedId(data.tasks[0]?.id ?? null); setToast("任务已从 Flight Deck 本地记录中删除。"); };
  ...
  const chooseProject = async (path) => { const response = await fetch("/api/projects/active", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ path }) }); const data = await response.json(); if (!response.ok) return setToast(data.error); setProject(data.project); setAllProjects(false); setProjectOpen(false); setToast(`当前项目：${data.project.name}`); };
  const pickProjectFolder = async () => { const response = await fetch("/api/projects/pick", { method: "POST" }); const data = await response.json(); if (!response.ok) return setToast("未选择文件夹。"); setProjectPath(data.path); };
  ...
  return <div className="app-shell">
    <section className="workspace">
      <header className="topbar"><div className="project-wrap"><button className={`project ${projectOpen ? "open" : ""}`} onClick={() => setProjectOpen((open) => !open)} title={allProjects ? "显示所有项目任务" : project?.path}><span className="project-mark"></span>{allProjects ? "全部项目" : project ? project.name : "正在读取项目…"}<small>{!allProjects && project?.branch ? ` · ${project.branch}` : ""}</small><span className="chevron">⌄</span></button>{projectOpen && <div className="project-menu" role="menu"><p>本机项目</p><button role="menuitem" className={allProjects ? "chosen" : ""} onClick={() => { setAllProjects(true); setProjectOpen(false); }}><span className="project-mark"></span>全部项目{allProjects && <b>✓</b>}</button>{projects.map((item) => <button role="menuitem" className={!allProjects && project?.path === item.path ? "chosen" : ""} key={item.path} onClick={() => chooseProject(item.path)}><span className="project-mark"></span>{item.name}{!allProjects && project?.path === item.path && <b>✓</b>}</button>)}<div className="project-divider"></div><button className="new-project" onClick={() => { setProjectOpen(false); setProjectModalOpen(true); }}>+ 添加项目</button></div>}</div><label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索任务…" /></label></header>
      {view === "tasks" && <main className="content">
        ...
        <aside className="inspector">
          <button className="close" title="关闭详情" onClick={() => setToast("详情面板已固定显示，便于快速审批。")}>×</button>
          ...
          {selected.status === "待复核" ? <div className="actions"><button className="primary success" onClick={acceptDelivery}>✓ 接受交付</button><button onClick={requestChanges}>请求补充</button><button className="danger-action" onClick={deleteSelectedTask}>删除任务</button><small>验证、diff 与风险说明会随交付保留。</small></div> : <div className="actions">{selected.status === "执行中" ? <>{selected.codex?.state === "running" ? <button className="primary" disabled>Codex 正在执行…</button> : <button className="primary success" onClick={markTestsPassed}>✓ 标记测试通过</button>}<button onClick={requestChanges}>停止并修改计划</button></> : <><button className="primary" disabled={selected.canRun === false || launching} onClick={launchTask}>{selected.canRun === false ? "等待依赖门禁" : launching ? "正在启动 Codex…" : "▷ 批准计划并启动"}</button><button onClick={requestChanges}>请求修改</button></>}<button className="danger-action" onClick={deleteSelectedTask}>删除任务</button><small>{selected.canRun === false ? "前置任务未满足测试或验收要求。" : "Codex 会在新的 worktree 中运行。"}</small></div>}
        </aside>
      </main>}
    ...
    {projectModalOpen && <div className="modal-backdrop" role="presentation"><section className="modal project-modal" role="dialog" aria-modal="true" aria-labelledby="project-title"><button className="modal-close" onClick={() => setProjectModalOpen(false)}>×</button><div className="modal-kicker">接入本机项目</div><h2 id="project-title">添加项目</h2><p className="description">选择任意本机文件夹。有 Git 提交时，任务会创建独立 worktree；没有提交或未启用 Git 时，Codex 会在原项目目录执行，并标记为共享工作目录。</p><div className="form-stack"><label>项目文件夹<input value={projectPath} onChange={(event) => setProjectPath(event.target.value)} placeholder="点击右侧选择文件夹" /><button className="outline" onClick={pickProjectFolder}>选择文件夹…</button></label></div><footer className="modal-actions"><button className="outline" onClick={() => setProjectModalOpen(false)}>取消</button><span></span><button className="primary" onClick={addProject} disabled={!projectPath}>验证并添加</button></footer></section></div>}
