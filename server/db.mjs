...
const roles = new Set(["产品专家", "前端专家", "后端专家", "UI/UX 专家", "全栈工程师", "测试专家", "DevOps 专家", "安全专家", "数据工程师"]);
...
export function createTask(input) {
  ...
  const task = { id, title, role, description: input.goal?.trim() || "等待补充交付目标。", projectPath: input.projectPath || "", status: "待开始", worktree: `wt/${title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "new-delivery"}`, activity: `等待 ${role} 启动`, test: "未运行", testTone: "neutral", files: ["AGENTS.md", "README.md", "相关业务模块（待扫描）"], plan: ["阅读已选上下文与项目约定", "实施目标范围内的最小变更", "运行验收标准要求的验证"], approved: false, acceptance: input.acceptance || "", merge: { mode: input.mergeMode === "auto" ? "auto" : "manual", targetBranch: input.targetBranch || "", state: "pending" } };
...
export function deleteTask(id) { const task = getTask(id); if (!task) throw new Error("Task not found"); if (["执行中", "已完成"].includes(task.status)) throw new Error("运行中或已完成的任务会保留为交付记录，不能删除。"); db.prepare("DELETE FROM dependencies WHERE dependent_id = ? OR prerequisite_id = ?").run(id, id); db.prepare("DELETE FROM tasks WHERE id = ?").run(id); return task; }
export function approveTask(id) { const task = getTask(id); if (!task) throw new Error("Task not found"); if (!["待开始", "计划中"].includes(task.status)) throw new Error("当前状态不能启动 Codex。"); if (!canRun(id)) throw new Error("任务依赖尚未满足：必须等待前置任务的验证门禁通过。"); return save({ ...task, approved: true, status: "计划中", activity: "计划已确认，等待启动 Codex", test: "等待执行", testTone: "neutral" }); }
export function recordCodexLaunch(id, launch) {
  const task = getTask(id); if (!task) throw new Error("Task not found");
  if (!["待开始", "计划中"].includes(task.status)) throw new Error("当前状态不能再次启动 Codex。");
  const now = new Date().toISOString();
...
export function stopCodexTask(id) { const task = getTask(id); if (!task) throw new Error("Task not found"); if (task.status !== "执行中" || task.codex?.state !== "running") throw new Error("当前没有可停止的 Codex 会话。"); const now = new Date().toISOString(); return save({ ...task, status: "计划中", approved: false, activity: "已停止 Codex，本轮变更仍保留在任务 worktree 中", codex: { ...task.codex, state: "stopped", stoppedAt: now }, execution: { ...(task.execution || {}), phase: "已停止", updatedAt: now, events: [...(task.execution?.events || []), { at: now, kind: "stopped", label: "已停止 Codex 执行", detail: "未自动删除 worktree 或任何代码变更" }].slice(-40) } }); }
...
export function recordMergePreview(id, preview) { const task = getTask(id); if (!task) throw new Error("Task not found"); return save({ ...task, merge: { ...(task.merge || {}), ...preview, state: preview.state, previewedAt: new Date().toISOString() } }); }
export function recordMergeResult(id, result) { const task = getTask(id); if (!task) throw new Error("Task not found"); if (task.status !== "待复核" || task.testTone !== "success") throw new Error("只有真实验证通过、等待复核的任务可以合并。"); return save({ ...task, activity: `已合并到 ${result.targetBranch}，等待人工确认交付`, merge: { ...(task.merge || {}), ...result, state: "merged" } }); }
export function recordVerification(id, verification, mergePreview = null) { const task = getTask(id); if (!task) throw new Error("Task not found"); if (task.codex?.state !== "completed") throw new Error("请等待 Codex 本轮执行完成后再运行验证。"); const success = verification.available && verification.exitCode === 0; const automatic = success && task.merge?.mode === "auto" && mergePreview?.state === "merged"; const updated = save({ ...task, status: success ? "待复核" : "已阻塞", activity: success ? automatic ? `真实验证通过，已自动合并到 ${mergePreview.targetBranch}` : "真实验证已通过，等待查看变更并确认合并" : verification.missingDependency ? "worktree 缺少项目依赖，等待准备后重试" : "验证未通过或未配置，等待处理", test: success ? `${verification.command} 通过` : verification.missingDependency ? "依赖未准备，无法验证" : verification.available ? `${verification.command} 失败（退出码 ${verification.exitCode}）` : "没有可运行的验证命令", testTone: success ? "success" : "danger", merge: mergePreview ? { ...(task.merge || {}), ...mergePreview, state: mergePreview.state } : task.merge, evidence: { ...(task.evidence || {}), verification, verifiedAt: new Date().toISOString() } }); if (success) releaseDependents(id); return updated; }
export function acceptTask(id) { const task = getTask(id); if (!task) throw new Error("Task not found"); if (task.codex?.isolated && task.testTone === "success" && task.merge?.state !== "merged") throw new Error("请先查看变更并确认合并到主项目，再接受交付。"); if (!["待复核"].includes(task.status)) throw new Error("当前状态不能接受交付。"); const updated = save({ ...task, status: "已完成", activity: task.merge?.state === "merged" ? `交付已接受 · 已合并到 ${task.merge.targetBranch}` : "交付已接受", test: "验证通过", testTone: "success" }); releaseDependents(id); return updated; }
export function returnTask(id) { const task = getTask(id); if (!task) throw new Error("Task not found"); if (["执行中", "已完成"].includes(task.status)) throw new Error("运行中或已完成的任务不能退回计划。"); return save({ ...task, status: "计划中", approved: false, activity: "等待你补充修改意见" }); }
